var qsConsole;
    ...
    function _execScript(cmd){
        ...
        if(cmd == "help"||cmd == "h"){
            ...
            clear();
        }else if(cmd == "hide" || cmd == "exit"){//隐藏控制台
            hide();
        }else{
            var _rv = '<p style="color:#ccff00">' + cmd + '</p>';
            try {
    ...
    qsConsole.init = function() {
        show();
        qsConsole.error( [ '<b>', NAMESPACE, '</b> can only be initialized once' ] );
    }
